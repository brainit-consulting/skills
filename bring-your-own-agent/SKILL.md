...
description: Add agent access to an app that already exists, so tools like Claude can do its real work — reading it and changing it the way the owner would. Use when someone wants Claude to work with an app they already have, rather than building a new one. Reads the app to find what it can do, agrees a list of capabilities with the owner, then generates a small MCP server that sits beside the app and talks to the app's own API. Adds one folder and edits nothing they wrote. It suits server-rendered and API-first apps best — Django, Express and FastAPI, where route discovery is written down and verified, and where the app already exposes an API meant for non-browser clients. The Next.js App Router is covered on the evidence of one real app, but that family keeps most of its work in Server Actions that cannot be called from outside, so an app with proper sign-in should usually build agent access inside itself instead; this skill says so first and builds it here only if the owner still wants that. Other stacks are read the same way, and where there is no verified playbook it says so rather than guessing.
---
