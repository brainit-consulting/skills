...
**Understanding comes before scaffolding.** The interview is the most valuable part of this skill, not a formality to get through. Ten minutes of good questions produces an app the user recognises; skipping them produces a generic CRUD shell they have to rewrite. Nothing is installed, created or written until Step 3 is agreed. Read-only checks are the exception and are allowed at any point, because several interview answers depend on them: `docker info`, `docker ps`, `vercel whoami`, `gh auth status`, listing the current folder. They change nothing, and they stop you promising a database or a deploy the machine can't deliver.
...
2. **"Do people need their own login?"**
   → This question settles the **access shape**, one of four, and every reference file uses these names:
     - **open sign-up** — anyone can create an account. A public product.
     - **one owner** — sign-up closes after the first account. A personal tool, or a public site with one author.
     - **invited people only** — the owner invites; strangers cannot sign up. A team or staff tool.
     - **no accounts** — skip auth entirely.
   → **The rule that decides it: if anyone has to add or edit what the app shows, someone needs a way in.** A public site with one author — poems, a portfolio, a menu — is **one owner** with a public front, not no accounts; otherwise the author has no way to publish. A tool used by one person on their own is **one owner** if it is ever reachable by others, which means deployed; it is **no accounts** only if it truly stays on their machine. Work this out from Step 1b's answer and confirm it in passing rather than reciting the four names at them.
   → Where the team has more than "owner and everyone else" — an office, plumbers, an owner — write the real role names on the build sheet. `references/auth.md` has the design for roles and invitations, and says plainly that it has not yet been built with this skill.
   → Any shape with accounts: recommend **email + password** as the default ("works immediately, nothing to configure").
...
**Dispatch one subagent per chosen branch, all in a single message so they run at once.** Only the branches the interview selected — there is no sense researching payments for an app that takes no money. The base project, the database, the pages step and discoverability always count as branches here. Design, legal, settings and system visibility (`references/design.md`, `legal.md`, `settings.md`, `ops.md`) always run too, so they count as well — but they install little of their own and mostly use what the other branches research, so one subagent can take all four as a light confirmation pass: the Better Auth options `settings.md` uses, the font and theme conventions `design.md` writes, and anything either file's `Last verified` date makes doubtful.
...
**The public-documentation branch, where it was chosen, gets one extra line in its brief:** *return the current package names for MDX in Next — the packages to install and the config entry that enables them — rather than confirming the ones `references/docs.md` lists.* Those names have moved before, and a brief that only asks "is this still current" tends to get a yes.
...
> **Signing in:** email and password. The first account is yours, and sign-up closes after it, so nobody else can get in.
...
- The **signing in** line names the access shape from question 2 in their words: anyone can sign up; only you; only people you invite, with the roles named; or no sign-in at all. For one owner and invited people only, say that the first account created is theirs — it is why nobody else signs up before they do.

One more line appears only where question 12's sub-question was asked — a site whose content is the product:

- The **AI crawlers** line states the decision both ways round: *"search and AI assistants can find and quote your poems; crawlers that collect text to train models are asked to stay out"*. If they refused training, the same line says whether the site still publishes an `llms.txt` index of its content, because an author who said no to training may not want one. `references/seo.md` has the call. Say once that `robots.txt` is a request a crawler may ignore.
...
Three rules hold for the whole of this step:

- **Commit after each step.** `references/stack.md` runs `git init -b main` and makes a first commit right after the scaffold; from then on, one commit per numbered step below, once its Verify has passed. `references/verify.md` reads the current commit and checks `drizzle/` for uncommitted changes, and neither check means anything in a project with no commits. This is local history only — nothing is pushed, and GitHub is still an offer at hand-off.
- **No account is created before Step 6.** Not to try the sign-up form, not to check a guard. The first account becomes the owner, and on a one-owner app a test account takes the user's seat and locks them out of their own app. Any Verify item that needs a signed-in account is written as *"Deferred to Step 6, after the user has signed up"* — leave it there. Checks that need no account stay where they are: types compile, a protected route redirects when signed out, a refused POST is refused.
- **Verify only what can be checked at that point.** Some Verify items need the finished app: uploading through the app's own UI, triggering the real job, gating the real feature behind a plan. Those pages don't exist until Step 5, so the item is carried to Step 5 or Step 6 and named there, rather than ticked from memory or satisfied with a throwaway page.

1. Base project → `references/stack.md`
2. Design system → `references/design.md` (always; a short `DESIGN.md` even when the user gave no direction)
3. Database (SQLite or Postgres branch) → `references/database.md`
...
- The schema tables are the **nouns** from Step 1a, each with a UUID primary key, and the ownership rule from Step 1b applied. Ownership is one of three cases, and `references/database.md` and `references/pages.md` use the same three:
  - **Private to each user** — a `userId` column (`text`, matching Better Auth) with `onDelete: "cascade"`, and every query scoped to the user from the session.
  - **Shared by a team** — no per-user scoping. Every query is scoped by **role** instead: a plumber sees the jobs assigned to them, the office sees all of them. A column that points at a person (assigned to, created by) uses `onDelete: "set null"` or `"restrict"`, never cascade, or a member who leaves deletes the team's history with them.
  - **No accounts** — no `userId` column at all.
- **Limits and permissions are enforced in the shared `src/lib/<domain>` functions, never only in a page.** Who may do this, how many the plan allows, which role may see it: the check lives in the function that the page, the server action and any agent tool all call. A limit written into a page is one an agent tool call walks straight past.
...
- Pick up the Verify items Step 4 carried forward because they needed these pages — the upload through the real form, the real job triggered, the real feature gated. The ones that need no account are checked here; the ones that need a signed-in account wait for Step 6. Still no account is created in this step.
- Done when: someone opening the app would know what it is without being told, and the user can do the main thing the app exists for, end to end. Where that needs a signed-in account, Step 6 is where it is shown, after they have signed up.
...
`references/verify.md` has the commands: types, schema drift, the build, lint, the app served in production mode, every route answering, the account checks for this app's access shape, and the app with its keys taken away. The account checks differ by shape: open sign-up sets two probe accounts against each other; one owner can have no probe accounts, so the pass is a refused second sign-up with the user count unchanged, a 404 on an unknown id, and a read of every query, and the two-account probes are named at hand-off as not applicable; invited people only creates its fixtures through the app's own invite function and tests role against role. Read the output of each. Having written the code a command tests is the reason to run it, not a reason to skip it.
...
- **The user signs up before any probe account exists.** No account has been created up to this point, and the first one is theirs: ask them to sign up now, in the running app, and wait until they have. The first account created becomes the admin. A fixture that takes that place, and is then deleted, locks the user out of their own system page. On a one-owner app it is worse: the fixture closes sign-up before the user has an account at all. The Verify items that Steps 4 and 5 deferred to here are run after that sign-up.
...
- **The database is not optional, so it does not degrade.** With `DATABASE_URL` missing the app fails immediately and says what's wrong in one plain sentence — on the hosted branch, *"the database isn't connected yet: run `vercel env pull .env.local`"*. An app that starts, looks fine, and then errors on the first click is worse than one that refuses to start, because it sends the user looking in the wrong place. Where there is sign-in, `BETTER_AUTH_SECRET` is the second required key and is never blanked in this check either: measured in a real build, a blank secret in production mode makes every auth page answer 500.
- **The real thing, end to end.** `drizzle/` contains generated migration files, no schema was ever pushed, and creating one real record works. With sign-in: signing up, signing out and signing back in works, and the app's protected route — `/dashboard` for a product, `/` for a personal tool with sign-in, which has no `/dashboard` — redirects when signed out and shows their own data when signed in. With uploads: a file uploaded through the app's own UI renders after a refresh. With payments: the test-mode checkout completes and the paid state is visible server-side. `DESIGN.md` exists, `globals.css` matches it — in light only by default, and in dark as well only where `DESIGN.md` calls for dark mode and something in the app switches it on — and no component sets a colour outside the tokens. With the help guide: `?` opens it, every verb from Step 1a has a chapter, and it reopens where the user left it.
...
- Every check that could not be run is named, with what it would need — a browser, a domain, a payment key — as a short list they can work through in a minute once the app is open. A check that does not apply is named too, with the reason: on a one-owner app the two-account isolation probe and the non-admin system-page probe are "not applicable: only one account can exist". Findings left unfixed after Step 7, and any finding you disagreed with, go here too, one line each with the reason.
...
- Discoverability, where the AI-crawler question was asked: repeat the decision from the build sheet in one line — which crawlers are allowed, which are asked to stay out, and whether there is an `llms.txt` — and say that `robots.txt` is a request a crawler may ignore, not a lock. Name `src/app/robots.ts` as where to change it.
...
- **Then offer the things you deliberately didn't do**, one line each, and do none of them without a clear yes:
  ...
  - *"Want me to put it online so you can look at it on your phone?"* → `references/deploy.md`. Say what a preview URL is in one sentence: a real web address, live now, that anyone with the link can open. That last part matters to a business owner and takes four words.
    ...
    **The live site starts with an empty database, so the first sign-up on the live address becomes the owner.** For one-owner and invited-people-only apps, never sign up on the live site yourself, not even to check it works. The user signs up there first, immediately after the deploy and before the address is shared with anyone. Say this when you make the offer, not after the deploy. Production's `DATABASE_URL` comes from the hosting integration's production environment, never copied from `.env.local`, which holds the development copy.
  - If the reason for putting it online is to **show** the app to someone — a client, a prospect, a colleague — rather than to use it, that is a different job: `references/demo.md` adds a way in that doesn't hand over the user's own data. The shared demo login exists only on an **open sign-up** app: on a one-owner or invited app a second account cannot be created, so there the demo is the sample-data script alone, shown by the person from their own account. Load it *before* `deploy.md`, because it introduces a build-time variable that has to be set before the build runs. Its demo login is only ever created on a database that already has an admin, so it is an ordinary user and cannot open the system page — which is one more reason the user signs up on the live site first. Offer it only when the user says the deployment is for showing; a demo nobody asked for is scaffolding to delete later.
