...
Where the app has a single owner or is invite-only, this hook is also where sign-up closes after the first account. `references/auth.md` has that version. It is one hook, not two.
...
`references/pages.md` already wrote `src/lib/auth-guards.ts` with a cached `getSession()`, `requireUser()` and `requireUserAction()`. Add the admin check to that file rather than starting a second one. If the file is somehow missing, the version below stands alone:
...
One row per granted consent, read through the OAuth provider plugin's client: `authClient.oauth2.getConsents()`, with `oauthProviderClient()` from `@better-auth/oauth-provider/client` added to the auth client's plugins:
...
- **Revoke**, calling `authClient.oauth2.deleteConsent({ id })`.

Revoking has to break the *next* tool call, not just remove a row from this page. **Deleting the consent does not do that by itself.** Access tokens are signed notes checked without asking the database, so one already issued keeps working until it expires, up to an hour after the user believed they had cut it off. The fix lives at the endpoint, not on this page: `references/mcp.md` has the route check that looks the consent up on every call. Confirm it against a live connection rather than assuming, and do not write "access stops immediately" on this page until that test has been run.
