...
`pnpm email:dev` opens a preview at http://localhost:3001 (its default port is the same as the app's, hence the flag) with a mobile toggle, the plain-text version, and a spam check. It's the fastest way to iterate, and it sends nothing.
