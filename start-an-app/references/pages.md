...
Write that helper now, in `src/lib/auth-guards.ts`, because this is the first step that needs it. `references/settings.md` adds `requireAdmin()` to the same file later and does not redefine this one:
