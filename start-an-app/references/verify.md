...
- **`BETTER_AUTH_URL` is not one of them: blank it, and expect the app to cope.** Measured on a build with agent access: passing the empty value straight into `baseURL` took every page behind sign-in, both discovery documents and `/mcp` to `500`, while the public pages stayed up. If this sweep shows that pattern, `references/mcp.md` has the fix (`baseURL` shares the `BASE_URL` fallback).
